...
class AuthServices {
    constructor() { }
    userRepo = new index_1.UserRepository();
    signup = async (req, res, next) => {
        try {
            ...
            const user = await this.userRepo.createUser({
                ...
                lastName,
                email,
                password: await (0, index_3.createHash)(password),
                gender: gender || index_2.Gender.male,
                phone,
                emailOtp: {
                    otp: await (0, index_3.createHash)(confirmEmailOtp),
            ...
            return (0, index_3.successHandler)({ res, msg: "User created successfully", status: 201 });
        }
        catch (error) {
            next(error);
        }
    ...
    confirmEmail = async (req, res, next) => {
        try {
            ...
            if (!user) {
                throw new index_3.userNotFoundError();
            }
    ...
    login = async (req, res, next) => {
        try {
            ...
            const { accessToken, refreshToken } = (0, index_3.generateToken)({ payload: { id: user._id } });
            (0, index_3.successHandler)({ res, msg: "logged in successfully", data: { accessToken, refreshToken }, status: 200 });
        }
        catch (error) {
            next(error);
        }
    ...
    refreshToken = async (req, res, next) => {
        try {
            ...
            const { user, decodedToken } = await (0, auth_middleware_1.decodeToken)({ authorization, tokenType: index_2.TokenType.refresh });
            const accessToken = (0, index_3.generateAccessToken)({ payload: { id: user._id }, jwtid: decodedToken.jti });
            (0, index_3.successHandler)({ res, msg: "refresh token generated successfully", data: { accessToken }, status: 200 });
        }
        catch (error) {
            next(error);
        }
    ...
    forgotPassword = async (req, res, next) => {
        try {
            ...
            return (0, index_3.successHandler)({ res, msg: "Reset Password Email Sent Successfully", status: 200 });
        }
        catch (error) {
            next(error);
        }
    ...
    resetPassword = async (req, res, next) => {
        try {
            ...
            user.password = await (0, index_3.createHash)(password);
