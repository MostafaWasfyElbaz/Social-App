...
router.use(routes.getChat, chatRouter);
...
router.patch(
  routes.acceptFriendRequest,
  auth(),
  validationMiddleware(acceptFriendRequestSchema),
  userServices.acceptFriendRequest
);
router.patch(
  routes.deleteFriendRequest,
  auth(),
  validationMiddleware(deleteFriendRequestSchema),
  userServices.deleteFriendRequest
);
router.patch(
  routes.unfriend,
  auth(),
  validationMiddleware(unfriendSchema),
  userServices.unfriend
);
router.patch(
  routes.blockUser,
  auth(),
...
router.patch(
  routes.uploadProfilePicture,
  auth(),
  uploadFile({}).single("profilePicture"),
  userServices.uploadProfilePicture
);
router.patch(
  routes.uploadCoverImages,
  auth(),
  uploadFile({}).array("coverImages", 5),
  userServices.uploadCoverImages
);
router.patch(
  routes.uploadFileWithPresignedUrl,
  auth(),
  userServices.uploadFileWithPreSignedUrl
);
router.patch(
  routes.updateUserBasicInfo,
  auth(),
  userServices.updateUserBasicInfo
);

router.get(routes.getFilesOrDownload, userServices.getFilesOrDownload);
router.get(
  routes.getFilesOrDownloadPreSignedUrl,
  userServices.getFilesOrDownloadPreSignedUrl
);
router.get(routes.getUserProfile, auth(), userServices.getUserProfile);
